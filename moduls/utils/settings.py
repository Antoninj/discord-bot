...
class Settings:
    ...
    def __init__(self, path=default_path, parse_args=True):
        ...
        self.default_settings = {
            "TOKEN": "MzE4MDcxOTU0ODk0NDg3NTY0.DAtDHA.5YbaTYhD26K-hIxb-oh815r3Glk",
            "EMAIL": None,
            "PASSWORD": None,
            "OWNER": "286246724270555136",
            "PREFIXES": [],
    ...
    def parse_cmd_arguments(self):
        parser = argparse.ArgumentParser(description="MasterBot")
        parser.add_argument("--owner", help="ID of the owner. Only who hosts "
                                            "Master should be owner, this has "
        ...
        parser.add_argument("--admin-role", help="Role seen as admin role by "
                                                 "Master")
        parser.add_argument("--mod-role", help="Role seen as mod role by Master")
        ...
        parser.add_argument("--no-cogs",
                            action="store_true",
                            help="Starts Master with no cogs loaded, only core")
        parser.add_argument("--self-bot",
                            action='store_true',
                            help="Specifies if Master should log in as selfbot")
        ...
        parser.add_argument("--dry-run",
                            action="store_true",
                            help="Makes Master quit with code 0 just before the "
    ...
    def check_folders(self):
        folders = ("data", os.path.dirname(self.path), "moduls", "moduls/utils")
    ...
    @property
    def token(self):
        return os.environ.get("MASTER_TOKEN", self.bot_settings["TOKEN"])
    ...
    @property
    def email(self):
        return os.environ.get("MASTER_EMAIL", self.bot_settings["EMAIL"])
    ...
    @property
    def password(self):
        return os.environ.get("MASTER_PASSWORD", self.bot_settings["PASSWORD"])
